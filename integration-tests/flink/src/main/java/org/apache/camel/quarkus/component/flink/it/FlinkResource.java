...
public class FlinkResource {
    ...
    public Response dataSetFromTextFile(@PathParam("fileName") String fileName) throws Exception {
        if (Files.exists(Paths.get(fileName))) {
            DataSet<String> myDataSet = Flinks.createExecutionEnvironment().readTextFile(fileName);
    ...
    @Consumes(MediaType.TEXT_PLAIN)
    @Produces(MediaType.TEXT_PLAIN)
    public Response loadStream(String data) throws Exception {
        StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment();
        DataStream<String> datastream = env.fromElements(data);
        context.getRegistry().bind("myDataStream", datastream);
        final java.nio.file.Path output = Files.createTempFile("camel", "txt");
